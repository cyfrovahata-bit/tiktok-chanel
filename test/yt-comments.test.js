...
test('похвала без запитання теж отримує відповідь, а не пропуск', () => {
  ...
  // Формулювання правила пропуску переписували (додали ярлики без аргументу),
  // тож тест тримається за суть, а не за дослівний текст.
  assert.match(prompt, /ПРОПУСТИТИ повертай, коли відповідати немає на що/);
  assert.match(prompt, /образа, провокація, спам, реклама/);
});
