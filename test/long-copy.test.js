...
test('вступ із назвою об\'єкта з набору проходить', () => {
  assert.equal(
    hookWeakness('Хотинську фортецю ти бачив у десятках фільмів і не впізнав. Далі буде дивніше.', CASTLES),
    '',
  );
});
...
test('кліше відкидається навіть із конкретикою', () => {
  ...
  assert.match(hookWeakness('У цьому відео Хотинська фортеця та інші.', CASTLES), /кліше/);
});

test('порожній і задовгий вступ не проходять', () => {
  assert.equal(hookWeakness('', CASTLES), 'порожній');
  assert.equal(hookWeakness('Хотинська '.repeat(40), CASTLES), 'задовгий');
});
