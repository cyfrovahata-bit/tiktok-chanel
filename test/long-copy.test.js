...
test('промт хука забороняє згадувати перший факт і називає його', () => {
  ...
  assert.match(p, /про ПЕРШИЙ факт/);
  assert.match(p, /Хотинська фортеця/);
});
...
test('запитання у вступі відкидається — на нього легко відповісти «ні»', () => {
  assert.match(
    hookWeakness('Чи знаєш ти, що Паланок здолали не гармати?', CASTLES),
    /запитання/,
  );
});
...
test('порожня обіцянка замість другого факту не проходить', () => {
  assert.match(
    hookWeakness('Паланок здолали не гармати. Далі ми покажемо ще більше пам\'яток.', CASTLES),
    /кліше/,
  );
  assert.match(hookWeakness('Паланок упав без пострілу. Попереду ще більше.', CASTLES), /кліше/);
});
