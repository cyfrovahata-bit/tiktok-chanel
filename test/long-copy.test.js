...
test('прев\'ю — вертикаль 9:16, обкладинка — портрет 4:5', () => {
  // Вертикальну обкладинку YouTube поставити не дає: будь-яку завантажену
  // картинку він зберігає як 16:9. А картку в стрічці вертикальному відео
  // малює 4:5 — саме в неї плакат і має лягти рівно. Боки до 16:9 додає код.
  assert.match(previewPromptVideo(SET), /1080×1920/);
  assert.doesNotMatch(previewPromptVideo(SET), /1280×720/);
  assert.match(previewPromptYouTube(SET), /1080×1350 \(4:5\)/);
  assert.doesNotMatch(previewPromptYouTube(SET), /1080×1920/);
});
...
test('обкладинка YouTube просить одного героя, а не колаж', () => {
  ...
  assert.match(p, /золот/i);
  assert.match(p, /чверть висоти плаката/);
});

test('обкладинка пояснює справжній розмір числами, а не образами', () => {
  const p = previewPromptYouTube(SET);
  assert.match(p, /76 пікселів/);
...
test('промт обкладинки забороняє домальовувати боки', () => {
  // Боки до 16:9 додає код розмитою копією кадру. Якщо художник намалює їх
  // сам, плакат стане ширшим за картку 4:5 — і краї зріже вже YouTube.
  const p = previewPromptYouTube(SET);
  assert.match(p, /СТРОГО ПО ЦЕНТРУ/);
  assert.match(p, /БОКИ НЕ МАЛЮЙ/);
  assert.match(p, /картку 4:5/);
