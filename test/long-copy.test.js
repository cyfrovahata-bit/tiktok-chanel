...
test('обидва прев\'ю просять вертикаль 9:16', () => {
  // Добірка вертикальна, тож і обкладинка YouTube мусить бути вертикальною:
  // горизонтальну він у стрічці ріже до центральної третини ширини, і від
  // напису лишається кілька літер посередині.
  assert.match(previewPromptVideo(SET), /1080×1920/);
  assert.doesNotMatch(previewPromptVideo(SET), /1280×720/);
  assert.match(previewPromptYouTube(SET), /1080×1920 \(9:16\)/);
  assert.doesNotMatch(previewPromptYouTube(SET), /1280×720/);
});
...
test("промт обкладинки тримає напис у центральній смузі", () => {
  // Тепер обкладинка вертикальна — але ту саму картинку YouTube показує й у
  // горизонтальних місцях (пошук, комп'ютер, «схожі»), лишаючи від неї
  // центральну смугу 1080×608. Напис поза цією смугою там зникне.
  const p = previewPromptYouTube(SET);
  assert.match(p, /СТРОГО ПО ЦЕНТРУ/);
  assert.match(p, /БЕЗПЕЧНА СМУГА/);
  assert.match(p, /1080×608/);
  assert.doesNotMatch(p, /Текст ліворуч/);
  // Стара 4:5-арифметика більше не діє — щоб її випадково не повернули.
  assert.doesNotMatch(p, /576/);
});
