...
test('у гілці промт дозволяє закрити суперечку про факт і відгукнутись на досвід', () => {
  ...
  assert.match(p, /ЦЕ РОЗМОВА У ГІЛЦІ/);
...
test('уся розмова їде в промт одним списком, від першої репліки', () => {
  const p = draftPrompt(THREAD_COMMENT, 'Facebook', null);
  assert.match(p, /РОЗМОВА У ГІЛЦІ \(від першої репліки до останньої\)/);
  assert.match(p, /1\. Оксана: А хіба вони не давніші\?/);
  assert.match(p, /2\. Петро: Насправді копали під забудову/);
  assert.match(p, /Остання репліка — від Петро/);
});

test('модель має відповідати РОЗМОВІ, а не останньому рядку', () => {
  const p = draftPrompt(THREAD_COMMENT, 'Facebook', null);
  assert.match(p, /ПРОЧИТАЙ ЇЇ ЦІЛКОМ/);
  assert.match(p, /ДЛЯ ВСІЄЇ РОЗМОВИ, а не відповіддю на/);
  assert.match(p, /Питання, яке поставили на початку/);
  assert.match(p, /додай те, чого в ній БРАКУЄ/);
});

test('наші власні репліки в гілці теж видно моделі', () => {
  const p = draftPrompt({
    ...THREAD_COMMENT,
    thread: [
      { author: 'Петро', text: 'Копали під забудову', ours: false },
      { author: 'Канал', text: 'Саме так, дякуємо', ours: true },
      { author: 'Оксана', text: 'А коли саме?', ours: false },
    ],
    author: 'Оксана',
    text: 'А коли саме?',
  }, 'Facebook', null);
  assert.match(p, /3\. Канал: Саме так, дякуємо/);
  assert.match(p, /4\. Оксана: А коли саме\?/);
});
...
test('у звичайному коментарі правил гілки немає', () => {
  ...
  assert.doesNotMatch(p, /ЦЕ РОЗМОВА У ГІЛЦІ/);
});
