...
test('обидва завдання обмежують пошук, а не шукають до посиніння', async () => {
  // Найдорожче в добі — не токени, а ліміт запитів ChatGPT. Одна генерація
  // сюжету плюс одна генерація фото вибирали п'ятигодинне вікно, бо перший
  // промт фактчекав ТРИ теми одразу й перевіряв кожен аспект окремим запитом.
  const read = async (file) => readFile(path.join(ROOT, 'prompts', file), 'utf8');

  const tema = await read(PUBLIC_PROMPTS['znaly/tema']);
  assert.match(tema, /БЮДЖЕТ ПОШУКУ/);
  assert.match(tema, /не більше ВОСЬМИ пошукових запитів/);
  // Фактчекається один кандидат, не трійка: два з трьох однаково йдуть у кошик.
  assert.match(tema, /До фактчеку бери ОДНОГО/);
  assert.doesNotMatch(tema, /До фактчеку бери три найкращі/);

  const foto = await read(PUBLIC_PROMPTS['znaly/foto']);
  assert.match(foto, /НЕ БІЛЬШЕ ТРЬОХ пошукових запитів/);
});
