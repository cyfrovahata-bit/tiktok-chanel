...
Ось тексти слайдів (по одному рядку на слайд). Озвуч КОЖЕН слайд ЙОГО Ж
текстом — рівно по одному реченню на слайд (${slideCount} речень), у тому ж
порядку, з тим самим змістом і словами. Адаптуй ЛИШЕ для читання ВГОЛОС:
...
• якщо номер стоїть із власною назвою — без відмінювання назви за числівником
  (не «шістнадцятий психа», а «астероїд Психея»).
НЕ переказуй по-своєму, НЕ додавай і НЕ пропускай нічого — це найголовніше.
Останній слайд (напр. «А ВИ ЗНАЛИ?») озвуч як «А ви знали?». Службові підписи
типу «підписуйся» НЕ озвучуй.
...
${script}`;
  ...
  // Кількість речень має дорівнювати слайдам (для прив'язки). Довжину НЕ ріжемо
  // — вона визначається слайдами.
  return enforceSentenceCount(narration, slideCount);
}
