...
function slideNarrationWordCap(slideCount) {
  const TARGET_VIDEO = 26; // ціль із буфером під жорсткі 29 с (варіативність темпу)
  const FADE = 0.5, PAD = 0.5, TEMPO = 1.4, SPEECH_RATE = 1.5; // слів/с (сирих)
...
// Якщо слайди багатослівні — злегка скорочує (прибирає зв'язки/зайве), щоб
// відео вклалось у ліміт, НЕ прискорюючи дикцію.
...
ДОВЖИНА: разом НЕ більше ${maxWords} слів. Якщо слайди багатослівні — трохи
СКОРОТИ кожне речення: прибери зв'язки й зайве («спочатку», «потім», «завдяки
цьому», «саме», «справжні», «фізично»), але ЗБЕРЕЖИ головний факт і всі числа.
НЕ прискорюй темп — просто менше слів.
...
${script}`;
  ...
  // Backstop: якщо все одно задовге — стискаємо під ліміт (щоб відео ≤ ~29 с).
  const trimmed = await enforceWordBudget(narration, maxWords);
  // Кількість речень має дорівнювати слайдам (для прив'язки).
