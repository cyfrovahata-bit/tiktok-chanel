...
export async function publishInstagramReel({ videoUrl, caption }, options = {}) {
  ...
  const container = await graphRequest(`${encodeURIComponent(igUserId)}/media`, {
    ...
    params: {
      ...
      share_to_feed: 'true',
      // Назва «оригінального звуку» ролика. Instagram створює аудіо-сутність і
      // для API-публікацій (звідси й можливість її перейменувати), а з її
      // сторінки глядач переходить до інших роликів із тим самим звуком — тож
      // безлика назва це втрачений місток між роликами. Перейменувати можна
      // лише один раз, тому задаємо одразу при створенні.
      // У Facebook аналога немає: у ребра video_reels параметрів для звуку
      // немає взагалі.
      audio_name: process.env.IG_AUDIO_NAME || 'Чи Ви Знали?',
