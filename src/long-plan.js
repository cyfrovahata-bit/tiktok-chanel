...
//   неділя  — збірка з 5 фактів
//   вівторок — збірка з 5 фактів
//   п'ятниця — збірка з 15 фактів
...
// Скільки фактів іде сьогодні: 5, 15 або null (не день збірки).
...
// завтра: інакше глядач побачив би той самий факт двічі — окремо й усередині
...
export function buildThemePrompt(items, size, { avoidTitles = [], loose = false } = {}) {
  ...
  // Тижнева добірка на 15 фактів однією темою не збирається — там правило
...
Бери найрізноманітніші й найцікавіші факти, щоб поруч не стояли два схожі.
Назва має бути ЗАГАЛЬНА, але щоразу НОВА за формулюванням.`
    : `ГОЛОВНЕ ПРАВИЛО: факти мають триматися купи спільною темою — епоха, стихія
...
справді набирається рівно ${size} фактів саме про це. Порахуй їх, перш ніж
...
фактів у форматі «ID | назва». Твоє завдання — зібрати добірку рівно з ${size} фактів.
...
ПІДПИС ДО КОЖНОГО ФАКТУ — 2–3 слова, які диктор скаже перед ним:
«Факт другий. Хотинська фортеця». Це назва самого об'єкта чи явища, а не
переказ факту і не його розгадка. Без лапок і без крапки в кінці.
...
// --- Чесна назва -------------------------------------------------------------
// Перший прохід моделі обирає факти Й одразу дає назву — і саме там ламається:
...
export function buildTitlePrompt(items, { avoidTitles = [] } = {}) {
  ...
  return `Ти редактор українського каналу «Чи Ви Знали?». Набір фактів для довгої
добірки ВЖЕ зібрано — міняти його не можна. Твоє завдання одне: дати цьому
набору назву, яка стоятиме на обкладинці.

НАБІР (${items.length} фактів, «ID | назва»):
${list}

ГОЛОВНА ВИМОГА — НАЗВА НЕ БРЕШЕ.
Назва мусить чесно накривати ВСІ ${items.length} фактів до одного. Якщо серед
