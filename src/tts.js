...
// Заміна працює по цілому слову, тож кожен відмінок треба мати в словнику
// окремим рядком. Перелічувати їх руками виявилося ненадійно: спершу
// «Зінов'євськом» лишився без наголосу, бо в списку був лише називний
// відмінок, потім те саме сталося зі «скнилівського». Тому відмінкові хвости
// для власних назв не пишемо руками — їх розгортає ця функція.
//
// stem — основа ВЖЕ з наголосом; ключ для пошуку виходить із неї зняттям
// акута. Апостроф у ключі стає класом ['’ʼ]: у сценаріях трапляються всі три
// символи, і без цього «Зінов'євськ» із фігурним апострофом не знаходився б.
function withEndings(stem, endings) {
  const key = stem.replace(/́/g, '').replace(/['’ʼ]/g, "['’ʼ]");
  return endings.map((end) => [`${key}${end}`, `${stem}${end}`]);
}

// Те саме, але для слів із рухомим наголосом, який у непрямих відмінках
// переходить на закінчення: краве́ць, але кравця́, кравці́в. Тут акут живе в
// хвості, а не в основі, тож знімати його треба саме з хвоста.
function withStressedEndings(stem, endings) {
  const key = stem.replace(/́/g, '').replace(/['’ʼ]/g, "['’ʼ]");
  return endings.map((end) => [`${key}${end.replace(/́/g, '')}`, `${stem}${end}`]);
}

// Чоловічі назви твердої групи: Житомир, Інгул, Кіровоград, Єлисаветград.
const NOUN_M = ['', 'а', 'у', 'ом', 'і', 'е', 'ові'];
// Прикметники на -ський/-цький: Житомирський, Кропивницький, Скнилівський.
const ADJ = ['ий', 'ого', 'ому', 'им', 'ім', 'а', 'ої', 'ій', 'у', 'ою', 'е', 'і', 'их', 'ими'];
// Назви областей на -щина: Житомирщина, Кіровоградщина.
const REGION = ['а', 'и', 'і', 'у', 'ою'];
...
const PRONUNCIATION_FIXES = [
  ...
  // Полі́сся — наголос на «лі». ElevenLabs читав «поліссЮ́». Форми виписані
  // окремо, бо заміна підставляє готовий рядок і закінчення з регексу не
  // переносить.
  ['поліссям', 'полі́ссям'],
  ['поліссю', 'полі́ссю'],
  ['поліссі', 'полі́ссі'],
  ...
  // «мухоловка»: в одному ролику слово прозвучало двічі й по-різному —
  // спершу «муХО́ловка», потім правильно «мухоло́вка». Кожне речення
  // синтезується ОКРЕМИМ запитом (synthesizeAligned), тож рушій щоразу
  // вгадує наголос заново — звідси й різниця в межах одного відео.
  // Словник прибирає цю лотерею. Форми окремими рядками: заміна віддає
  // готовий рядок, тож відмінок регексом не підхопиш.
  ['мухоловка', 'мухоло́вка'],
  ['мухоловки', 'мухоло́вки'],
  ['мухоловку', 'мухоло́вку'],
  ['мухоловці', 'мухоло́вці'],
  ['мухоловкою', 'мухоло́вкою'],
  ['мухоловок', 'мухоло́вок'],
  // Пала́нок — Мукачівський замок. Рушій читав «палано́к», з наголосом на
  // останній склад; норма — на «ла». У непрямих відмінках «о» випадає
  // (Пала́нок → Пала́нка), тож основа там інша й форми виписані окремо.
  ['паланок', 'пала́нок'],
  ['паланка', 'пала́нка'],
  ['паланку', 'пала́нку'],
  ['паланком', 'пала́нком'],
  ['паланкові', 'пала́нкові'],
  ['паланки', 'пала́нки'],
  // Житомир: рушій ставив наголос на перший склад («ЖИ́томира») — норма
  // Жито́мир. Для каналу про топоніми це не поодинокий випадок, а цілий клас:
  // назви міст читаються за здогадкою, і чути це лише в готовому ролику.
  ...withEndings('жито́мир', NOUN_M),
  ...withEndings('жито́мирщин', REGION),
  ...withEndings('жито́мирськ', ADJ),
  // Кропивницький: наголос на «и» третього складу — Кропивни́цький, як у
  // прізвищі Марка Кропивни́цького, від якого місто й дістало назву.
  ...withEndings('кропивни́цьк', ADJ),
  // Зінов'єв: наголос на «о» — Зіно́в'єв. Так подає Енциклопедія Сучасної
  // України: ЗІНО́ВʼЄВ Григорій Овсійович (esu.com.ua/article-16200).
  // Звідси й назва міста 1924–1934 років — Зіно́в'євськ.
  ...withEndings("зіно́в'єв", ['', 'а', 'у', 'им', 'і', 'е', 'ові']),
  ...withEndings("зіно́в'євськ", NOUN_M),
  ...withEndings("зіно́в'євськ", ADJ),
  // Далі — топоніми з ролика про п'ять назв Кропивницького. Рушій читав їх
  // навмання, бо це власні назви поза його словником. Наголоси звірені з
  // Енциклопедією Сучасної України (у ній наголос стоїть просто в заголовку
  // статті) — джерело для кожної групи вказано поруч.
  //
  // Інгу́л — наголос на «у» (esu.com.ua/article-13331, стаття «ІНГУ́Л»).
  ...withEndings('інгу́л', NOUN_M),
  // Єлисаветгра́д — наголос на останній склад (esu.com.ua/article-17635,
  // стаття «ЄЛИСАВЕТГРА́Д»). Не «ЄлисавЕ́тград», як читав рушій.
  ...withEndings('єлисаветгра́д', NOUN_M),
  ...withEndings('єлисаветгра́дськ', ADJ),
  // Єлизаве́та / Єлисаве́та — наголос на «е» передостаннього складу. Обидва
  // написання: фортеця в джерелах зветься і «святої Єлисавети», і
  // «святої Єлизавети», а сценарій може взяти будь-яке.
  ...withEndings('єлизаве́т', ['а', 'и', 'і', 'у', 'ою']),
  ...withEndings('єлисаве́т', ['а', 'и', 'і', 'у', 'ою']),
  // Кі́ров — наголос на «і» (esu.com.ua/article-7008, стаття «КІ́РОВ Сергій
  // Миронович»). Звідси й назви міста 1934–1939 років: Кі́рове, Кі́рово.
  ...withEndings('кі́ров', ['', 'а', 'у', 'им', 'і', 'е', 'о', 'ові']),
  // Кіровогра́д — наголос на останній склад (esu.com.ua/article-7014, стаття
  // «КІРОВОГРА́Д»), а не «КіровОград».
  ...withEndings('кіровогра́д', NOUN_M),
  ...withEndings('кіровогра́дськ', ADJ),
  ...withEndings('кіровогра́дщин', REGION),
  // Григо́рій — наголос на «о».
  ...withEndings('григо́рі', ['й', 'я', 'ю', 'єм', 'ї', 'йович', 'йовича']),
  // Скни́лів — наголос на «и», а не на «і» («СкнилІ́в», як читав рушій).
  // Так подає ВЕСУМ (goroh.pp.ua/Словозміна/скнилів) і так само наголошено
  // в статті Вікіпедії про село. Непрямі відмінки мають основу Скни́лов-
  // (Скни́лова, у Скни́лові), похідні наголос кореня зберігають.
  ...withEndings('скни́лів', ['', 'ом']),
  ...withEndings('скни́лов', NOUN_M),
  ...withEndings('скни́лівськ', ADJ),
  ...withEndings('скни́лівок', ['', 'а', 'у', 'ом', 'ові']),
  // Монастирча́ни і Стару́ня — сусідні села на Івано-Франківщині, обидва
  // з глядацьких замовлень. Наголоси за ВЕСУМ (goroh.pp.ua). Монастирчани
  // мають лише множину, тож набір закінчень свій.
  ...withEndings('монастирча́н', ['и', '', 'ам', 'ами', 'ах']),
  ...withEndings('стару́н', ['я', 'і', 'ю', 'ею']),
  // Ли́сенко — наголос на першому складі, хоч більшість прізвищ на -енко
  // наголошується на передостанньому (Шевче́нко, Петре́нко). Саме тому рушій
  // і помиляється: він іде за загальним зразком. Так подає Велика українська
  // енциклопедія: Мико́ла Віта́лійович Ли́сенко.
  ...withEndings('ли́сенк', ['о', 'а', 'у', 'ом', 'ові', 'и', 'ів']),
  // Мука́чево — наголос на «а» другого складу. Тут є розбіжність, і про неї
  // варто знати: частина мовознавців обстоює форму «Мукачеве» з наголосом на
  // третьому складі, але місцева й ефірна вимова — Мука́чево, і саме її
  // впізнає глядач. Обрано живий варіант.
  ...withEndings('мука́чев', ['о', 'е', 'а', 'у', 'і', 'ом', 'им']),
  ...withEndings('мука́чівськ', ADJ),
  // Скі́нчуватися — наголос на «кі», як у спорідненому закі́нчувати. Рушій
  // читав «скінчува́лись». Увага: це НЕ те саме, що «скінча́тися», у якого
  // наголос інший, тож форми на «-чув-» перелічені окремо.
  ...withEndings('скі́нчув', ['ався', 'алася', 'алась', 'алися', 'ались', 'алося',
    'алось', 'ати', 'атися', 'ав', 'ала', 'али', 'ало']),
  // За́мок (фортеця) і замо́к (на дверях) — різні слова, які відрізняються
  // ТІЛЬКИ наголосом. Рушій вибирає навмання, і в ролику про Острог вийшов
  // «замо́к». Для каналу про історію потрібне завжди перше значення, тож
  // фіксуємо його. Свідомий компроміс: якщо колись у сценарії трапиться
  // справжній замо́к на дверях, він теж прозвучить як фортеця — але для
  // історичних сюжетів це в рази рідший випадок.
  // Родовий тут ще й розрізняє слова сам: за́мку (фортеці) проти замка́.
  ...withEndings('за́мок', ['']),
  ...withEndings('за́мк', ['у', 'ом', 'ові', 'и', 'ів', 'ам', 'ах', 'ами']),
  // Не назва, але з того ж ролика: «після його опали». Рушій читав «о́пали»
  // (наче листя опало), тоді як тут іменник опа́ла — немилість.
  ['опала', 'опа́ла'],
  ['опали', 'опа́ли'],
  ['опалу', 'опа́лу'],
  ['опалі', 'опа́лі'],
  // Краве́ць — наголос на «е», а рушій раз у раз читав «кра́вець», за зразком
  // «ко́вбаса → ко́вбасі». У непрямих відмінках випадає «е» основи, і наголос
  // переходить на закінчення: кравця́, кравце́м, кравці́, кравці́в. Через це
  // одним рядком тут не обійтися — потрібні обидві основи.
  ['кравець', 'краве́ць'],
  ...withStressedEndings('кравц', ['я́', 'ю́', 'е́м', 'е́ві', 'і́', 'і́в', 'я́м', 'я́ми', 'я́х']),
  // Прикметник наголосу не пересуває: краве́цька майстерня, краве́цькі ножиці.
  ...withEndings('краве́цьк', ADJ),
  ['кравецтво', 'краве́цтво'],
  ['кравецтва', 'краве́цтва'],
  ['кравчиня', 'кравчи́ня'],
  ['кравчині', 'кравчи́ні'],
  ['кравчинею', 'кравчи́нею'],
  ['кравчиню', 'кравчи́ню'],
  // Левку́ — наголос на закінченні. Рушій читав «Ле́вку», за зразком кличного
  // відмінка. Додано на прохання власника; інші відмінки не чіпаємо, бо в
  // сценаріях поки траплялася саме ця форма.
  ['левку', 'левку́'],
  // Доне́ччина — наголос на «не», як у Доне́цьку, від якого назва й утворена.
  // Тут, на відміну від «Левку», наголос нерухомий і сидить в основі, тож
  // безпечно розгорнути всі відмінки одразу — тим самим набором закінчень, що
  // й Кіровогра́дщина.
  ...withEndings('доне́ччин', REGION),
];
...
// Числівники другого десятка, написані в сценарії СЛОВАМИ («у середині
// вісімнадцятого століття»), повз numbersToWords проходять як звичайний текст:
// той чіпає лише цифри. Тому наголоси, зашиті в TEENS/ORD_TEENS, на них не
// діяли — і рушій читав «вісімнадцЯ́того». Наголос у цих словах завжди на «а»
// в «надцять», у будь-якому відмінку й роді, тож одне правило покриває всі
// форми — від «одинадцять» до «дев'ятнадцятої». Сполука «надцят» більше ніде
// в мові не трапляється, тож хибних спрацювань немає. Уже наголошене
// «на́дцят» під регекс не підпадає: акут (U+0301) розриває послідовність.
const TEEN_STRESS = /на(дцят)/gi;

function stressTeens(text) {
  return text.replace(TEEN_STRESS, (m) => `${m.slice(0, 2)}́${m.slice(2)}`);
}
...
// Головний вхід. Повертає { voicePath, slideDurations }:
//  - slideDurations = масив тривалостей слайдів (с), коли озвучка прив'язана
//    до слайдів (відео підлаштовується під неї) — тоді монтаж має використати
//    ці тривалості;
//  - slideDurations = null для суцільної начитки (рівні слайди, старий шлях).
export async function synthesizeVoiceover(
  ...
  if (slideCount && sentences.length >= slideCount) {
    try {
      const slideDurations = await synthesizeAligned(groupSentences(sentences, slideCount), outputPath);
      return { voicePath: outputPath, slideDurations };
    } catch (error) {
      // Прив'язка не критична: якщо щось пішло не так — не лишаємо відео без
      // звуку, а відкочуємось на суцільну начитку (рівні слайди).
      console.error(`Прив'язка до слайдів не вдалась (${error.message}); суцільна начитка.`);
    }
  }
  await runEngines(text, outputPath, targetSeconds);
  await fitToVideo(outputPath, targetSeconds);
  return { voicePath: outputPath, slideDurations: null };
}
...
// Синтезує кожну групу-речення окремо на ФІКСОВАНІЙ швидкості ALIGN_TEMPO, а
// тривалість показу кожного слайда підлаштовує під довжину його репліки
// (slide_i = аудіо_i + FADE + SLIDE_PAD). Ставить кожну репліку на початок
// свого слайда (adelay) і змішує (amix). Повертає масив тривалостей слайдів,
// який монтаж використає для збірки відео під озвучку.
async function synthesizeAligned(groups, outputPath) {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'tts-aligned-'));

  // 1) Кожне речення: синтез → обрізати тишу з обох боків → фіксований темп.
  const clips = [];
  ...
  // 2) Тривалість слайда = довжина репліки + перекриття fade + пауза-подих.
  const slideDurations = clips.map((clip) => clip.duration + FADE_SECONDS + SLIDE_PAD);
  const offsets = slideOffsets(slideDurations); // час старту кожної репліки
  console.log(
    `Прив'язка до слайдів (${ALIGN_TEMPO}×): тривалості ${slideDurations.map((d) => d.toFixed(1)).join('/')} с`,
  );

  // 3) Ставимо кожну репліку на початок її слайда й змішуємо.
  const inputs = [];
  const filters = [];
  ...
  if (clips.length === 1) {
    ...
    const mixIns = clips.map((_, k) => `[a${k}]`).join('');
    filterComplex = `${filters.join(';')};${mixIns}amix=inputs=${clips.length}:normalize=0[out]`;
    mapLabel = '[out]';
  }
  await run('ffmpeg', [
    '-y', ...inputs,
