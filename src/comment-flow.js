...
export function draftPrompt(comment, platformLabel = '', post = null) {
  ...
  return [
    ...
    '',
    ...(comment.parentId ? [
      'ЦЕ РОЗМОВА У ГІЛЦІ, а не окремий коментар під дописом. Нижче — вся',
      'гілка від першої репліки до останньої.',
      '',
      'СПЕРШУ ПРОЧИТАЙ ЇЇ ЦІЛКОМ і визнач для себе: про що ця розмова, до чого',
      'люди дійшли, що лишилось невирішеним. Аж тоді пиши.',
      '',
      'Твоя репліка має бути доречною ДЛЯ ВСІЄЇ РОЗМОВИ, а не відповіддю на',
      'останній рядок. Питання, яке поставили на початку й не закрили, важливіше',
      'за останню репліку. Якщо розмова вже дійшла згоди — не тягни її назад.',
      '',
      'Підтримай розмову: додай те, чого в ній БРАКУЄ — факт, деталь,',
      'підтвердження того, хто має рацію. Не переказуй сказане ними й не підбивай',
      'підсумків уголос: це звучить як учитель, що підсумовує урок.',
      '',
      ...
      '',
      'Звертайся на ім\'я до того, чию думку підхоплюєш або кому відповідаєш — у',
      'гілці інакше не зрозуміло, кому адресовано.',
      '',
    ...
    '- «Чи Ви Знали?» (і «Чи Знали Ви?») — це НАЗВА нашої сторінки, зі знаком',
    '  питання в самій назві. Глядач, який її пише, звертається до каналу, а не',
    '  ставить запитання. «Чи Знали Ви? Дякую» — це подяка; відповідати на неї',
    '  «дякуємо за запитання» не можна;',
    ...
    '',
    ...(comment.parentId ? [
      // Уся гілка одним списком, від першої репліки до останньої: розмову
      // видно цілком, а не як «контекст» окремо від коментаря.
      'РОЗМОВА У ГІЛЦІ (від першої репліки до останньої):',
      ...[
        { author: comment.parentAuthor, text: comment.parentText },
        ...(comment.thread || []),
      ]
        .filter((r) => r.text)
        .map((r, i) => `${i + 1}. ${r.author}: ${r.text}`),
      '',
      `Остання репліка — від ${comment.author}. Відповідаєш у цю гілку.`,
      '',
    ] : [
