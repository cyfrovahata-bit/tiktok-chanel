...
// Скільки перших фрагментів слайда розглядаємо під обкладинку. Чотири — це
// ще перші секунди зачину, тобто якір, а не середина думки.
const COVER_SCAN = 4;
// Вага фрагмента для обкладинки — довжина його НАЙДОВШОГО слова. Проста сума
// літер тут не працює: «ВСІЙ КРАЇНІ» набирає стільки ж, скільки «ПРІЗВИЩЕ», бо
// два службові слова разом не коротші за одне змістовне. А от довге слово — це
// майже завжди іменник або власна назва, тобто те, за що чіпляється око в
// сітці профілю.
function coverScore(chunk) {
  const words = String(chunk).replace(/[^\p{L}\p{N}\s]/gu, ' ').split(/\s+/).filter(Boolean);
...
//
// Беремо середину фрагмента з ЯКОРЕМ — власною назвою об'єкта. Саме середину,
// а не початок: на краях фрагмент ще проявляється або вже згасає (\fad), і
// кадр вийшов би напівпрозорим.
//
// Спершу тут бралося довше з ДВОХ перших — мовляв, власна назва довша за
// службові слова. На каналі «Звідки ми» це дало сітку з обкладинок «ВСІЙ»,
// «РОКУ», «А ЗГОДОМ», «ЦЮ РОДОВУ»: фрагменти в нас по 1–3 слова, і два
// службові слова разом виграють у одного змістовного. Тепер дивимося на
// чотири перші фрагменти й беремо той, у якому найдовше СЛОВО.
export function coverTimestampMs(lines, durations) {
  ...
  // За рівної ваги лишаємо ранніший фрагмент: він ближчий до якоря зачину.
  const best = events.slice(0, COVER_SCAN)
    .reduce((a, b) => (coverScore(b.chunk) > coverScore(a.chunk) ? b : a));
